...
class BalansController extends Controller
{
    ...
    public function list(Request $request){
        ...
        ->select('kash_categors.text as mesto','summa')->where('balans.user_id', $request->user()->id)->get();
              $arr = [];
             $arr['success']=true;
            $arr['data']=$balans;
   ...
   $lastdate= BalansControl::orderBy('updated_at','DESC')->where('user_id', $request->user()->id)->first()->toArray();
   if(empty($lastdate)){
    return;
   }
   ...
   if(! $month==now()->month){
      $balans = Balans::select('kash_categor_id','summa')->where('user_id', $request->user()->id)->get()->toArray();
      for ($y=0; $y<now()->month-$month; $y++){
           ...
           ->select('kash_categors.text as mesto')->groupBy('mesto')->whereYear('balanscontrol.updated_at','=',$request->year)->where('balanscontrol.user_id', $request->user()->id)->get()->pluck('mesto')->toArray();
              $arr=[];$resultArr=[];
for ($x=0; $x<count($mesto); $x++) {
      $arr[$x] = DB::table('balanscontrol')
            ->join('kash_categors', 'kash_categors.id', '=', 'balanscontrol.kash_categor_id' )
            ->where('balanscontrol.user_id', $request->user()->id)->where('kash_categors.text',$mesto[$x])->select(  DB::raw('MONTH(balanscontrol.updated_at) as month'),  DB::raw("SUM(summa) as summa"))
