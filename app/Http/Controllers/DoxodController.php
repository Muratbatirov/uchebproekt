...
class DoxodController extends Controller
{
    ...
    if($request->param!='false' and $request->param ){
     ...
     $doxod = DB::table('doxod')
            ...
            ->where('doxod.user_id', $request->user()->id)->where('doxcategor.text', $request->param)
          ...
          ->join('kash_categors', 'kash_categors.id', '=', 'doxod.kash_categor_id' )
            ...
            ->join('users', 'users.id', '=', 'doxod.user_id' )
            ->select( 'doxod.id','doxcategor.text AS categorya',  'summa', 'kash_categors.text as mesto', 'doxod.updated_at' )
            ->where('doxod.user_id', $request->user()->id)->orderBy('doxod.updated_at', 'DESC')
           ->skip($request->start)
                ->take(10)
                ->get()->toArray();
                 $doxodcount = DB::table('doxod')
            ...
            ->join('users', 'users.id', '=', 'doxod.user_id' )
            ->select( 'doxod.id','doxcategor.text AS categorya',  'summa', 'kash_categors.text as mesto'  )
            ->where('doxod.user_id', $request->user()->id)
...
"data" => $doxodcombo
    ...
    $doxodcombo = DoxCategor::where('user_id', $request->user()->id)
               ->select('id','text')
              ->get()->toArray();
   return json_encode(array(
"success" => true,
"data" => $doxodcombo
    ...
    public function update(Request $request){
         ...
         $data= json_decode(stripslashes($request->data));
          ...
          ->first();
            $doxcat= DoxCategor::where('user_id', $request->user()->id )->where('text', $value->categorya)
      ->select('id')->first()->id;
   ...
   "success" => true,
    ...
    "success" => true
     ...
     public function create(Request $request){
        ...
        foreach ($data as $value){
          ...
          ->first();
         $doxod = new Doxod;
     $doxod->user_id = $request->user()->id;
     $doxod->summa = $value->summa;
     $d = new DateTime($value->updated_at, new DateTimeZone('Asia/Tashkent'));
       $d ->add(new DateInterval("PT5H"));
      $doxod->updated_at = $d;
      $doxcatid = DoxCategor::where('user_id', $request->user()->id )->where('text', $value->categorya)
      ->select('id')->first();
    ...
    public function chart(Request $request){
     ...
     $doxodsum= DB::table('doxod')->join('doxcategor', 'doxcategor.id', '=', 'doxod.doxcategor_id' )
            ->where('doxod.user_id', $request->user()->id)->select(  'doxcategor.text AS categorya',  DB::raw("SUM(summa) as summa"))
...
"data" => $doxodsum
     ...
     $doxodsum= DB::table('doxod')->join('doxcategor', 'doxcategor.id', '=', 'doxod.doxcategor_id' )
            ->where('doxod.user_id', $request->user()->id)->select(  DB::raw('MONTH(doxod.updated_at) as month'),  DB::raw("SUM(doxod.summa) as summa"))
